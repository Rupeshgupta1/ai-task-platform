...
def add_log(task_id, level, message):
    tasks_collection.update_one(
        ...
        {"$push": {"logs": {"level": level, "message": message, "timestamp": datetime.utcnow()}}}
    )
...
def process_job(job_data):
    ...
    update_task(task_id, {"status": "running", "startedAt": datetime.utcnow()})
    add_log(task_id, "info", "Task processing started (Python worker).")
    try:
        result = process_operation(operation, input_text)
        update_task(task_id, {"status": "success", "result": result, "completedAt": datetime.utcnow()})
